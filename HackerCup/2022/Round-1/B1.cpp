/*
 * Author: Ryoga.exe
 * Submitted at: 2022-09-11 19:34:35 (UTC+09:00)
 * Problem URL: https://www.facebook.com/codingcompetitions/hacker-cup/2022/round-1/problems/B1
 * Result: Accepted
*/
#include <bits/stdc++.h>
using namespace std;
...
auto main() -> int {
    ...
    for (int casenum = 1; casenum <= t; casenum++) {
        ...
        while(q--) {
            ll x, y;
            cin >> x >> y;
            ll nowX = mod(n * x * x) - mod(2 * x * sumT.first) + mod(sumT2.first);
            ll nowY = mod(n * y * y) - mod(2 * y * sumT.second) + mod(sumT2.second);
